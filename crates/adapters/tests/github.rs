...
fn maps_jobs_to_transitions_and_substitutes_repository() {
    ...
    assert_eq!(
        obs.records[0].effects[0].args.get("scope").map(String::as_str),
        Some("repo/widgets"),
        "repository_name must be substituted from the full name"
    );
    assert_eq!(
        obs.records[1].effects[0].args.get("digest").map(String::as_str),
        Some("sha256:AA11"),
...
fn reusable_workflow_job_names_resolve_to_the_declared_job() {
    ...
    let jobs = Jobs {
        jobs: vec![
            job(1, "Build, test, publish / build, test, package", "success", "2026-01-01T00:00:00Z"),
            job(2, "Build, test, publish / Run simple test", "success", "2026-01-01T00:01:00Z"),
            job(3, "Build, test, publish / Create release", "success", "2026-01-01T00:02:00Z"),
        ],
    };
    let obs = normalize(&run(), &jobs, &[], &map());
    let steps: Vec<String> = obs.records.iter().filter_map(|r| r.step.clone()).collect();
...
fn matrix_leg_parameters_are_stripped() {
    let jobs = Jobs {
        jobs: vec![job(1, "build (ubuntu-latest, 1.75)", "success", "2026-01-01T00:00:00Z")],
    };
    let obs = normalize(&run(), &jobs, &[], &map());
    let steps: Vec<String> = obs.records.iter().filter_map(|r| r.step.clone()).collect();
...
fn provenance_creates_an_edge_only_when_the_build_is_observed() {
    ...
    let with_real_build = vec![
        Fact { step: "build".into(), digest: Some("sha256:AA11".into()), scope: None, provenance: None },
        Fact {
            step: "publish".into(),
            digest: Some("sha256:AA11".into()),
            scope: None,
            provenance: Some(Provenance { build_digest: "sha256:AA11".into(), issuer: String::new() }),
        },
    ];
    let obs = normalize(&run(), &jobs, &with_real_build, &map());
    assert_eq!(obs.edges.len(), 1, "provenance naming an observed build links them");
    ...
    let with_phantom_build = vec![
        ...
        Fact {
            ...
            scope: None,
            provenance: Some(Provenance { build_digest: "sha256:BEEF".into(), issuer: String::new() }),
        },
    ];
